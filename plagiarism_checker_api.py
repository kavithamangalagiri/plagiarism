...
async def get_plagiarism_matrices(lesson_content_id: int):
    ...
    if not results:
        ...
        headers = {
            "Content-Disposition": f"attachment; filename={lesson_content_id}_lesson_plagiarism.csv"
        }
        ...
        return StreamingResponse(iter([csv_data]), headers=headers, media_type='text/csv')
